...
final class UiModeManagerService extends SystemService {
    ...
    private void applyGradientColor() {
        final Context context = getContext();
        int gradientColor = System.getIntForUser(context.getContentResolver(),
                System.GRADIENT_COLOR, 0xFF000000, UserHandle.USER_CURRENT);
        String gradientHex = String.format("%08x", (0xFFFFFFFF & gradientColor));
        String gradientVal = SystemProperties.get(GRADIENT_COLOR_PROP);
        if (!gradientVal.equals(gradientHex)) {
            SystemProperties.set(GRADIENT_COLOR_PROP, gradientHex);
            try {
                mOverlayManager.reloadAndroidAssets(UserHandle.USER_CURRENT);
                mOverlayManager.reloadAssets("com.android.settings", UserHandle.USER_CURRENT);
                mOverlayManager.reloadAssets("com.android.systemui", UserHandle.USER_CURRENT);
